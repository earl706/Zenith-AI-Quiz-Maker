...
export const STUDY_ALARM_MAX_MS = 30000;
export const DEFAULT_STUDY_ALARM_SOUND = 'chime';

export const STUDY_ALARM_SOUNDS = [
...
function resolveSoundId(soundId) {
	return ALARM_PATTERNS[soundId] ? soundId : DEFAULT_STUDY_ALARM_SOUND;
}

export function playStudyAlarmSound(soundId = DEFAULT_STUDY_ALARM_SOUND) {
	try {
...
export function previewStudyAlarmSound(soundId) {
	playStudyAlarmSound(soundId);
}

export function startStudyAlarm(soundId = DEFAULT_STUDY_ALARM_SOUND) {
	stopStudyAlarm();
	const id = resolveSoundId(soundId);
	const repeatMs = ALARM_REPEAT_MS[id] ?? ALARM_REPEAT_MS.chime;

	playStudyAlarmSound(id);
	alarmInterval = window.setInterval(() => playStudyAlarmSound(id), repeatMs);
	alarmTimeout = window.setTimeout(stopStudyAlarm, STUDY_ALARM_MAX_MS);
}

export function stopStudyAlarm() {
